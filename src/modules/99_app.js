...
/* ============ shell ============ */
const TABS=[["team","Team"],["scores","Scores"],["poll","Poll"],
...
function render(){
  ...
  el("app").innerHTML=`
    ...
    <main id="view">${inOff?offseasonScreen():
      ...
      (view==="team"?myTeamView():view==="scores"?scoresView():
       view==="poll"?pollView():view==="stand"?standingsView():dynastyView())}</main>
    </div>
  ...
  document.querySelectorAll(".subtab").forEach(b=>b.onclick=()=>{
    ...
    else if(b.dataset.p){postTab=b.dataset.p}
    else if(b.dataset.m){teamTab=b.dataset.m}
    else if(b.dataset.k){pollTab=b.dataset.k}
    render()});
